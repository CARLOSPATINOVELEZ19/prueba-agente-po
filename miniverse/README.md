...
El **Orquestador** (regla `.cursor/rules/00-swarm-orchestrator.mdc`) debe elegir el especialista según la actividad y, en **cada Task**, poner como **primera línea** del prompt el metadato `[miniverse:CLAVE]` (p. ej. `[miniverse:scout]`, `[miniverse:historian]`). Así Miniverse muestra **quién se activa** acorde al mapa de dominio → agente.
...
3. **`.cursor/hooks.json`** llama a `miniverse/scripts/cursor-miniverse-sync.mjs` en `subagentStart` / `subagentStop`:
...
Si `subagentStop` llega **sin texto de task**, el script usa el último especialista guardado en **`.cursor/.miniverse-last-subagent.json`** (escrito en `subagentStart`, ignorado por git) y luego lo borra, para no marcar `idle` en el agente equivocado (p. ej. Scout por defecto).
