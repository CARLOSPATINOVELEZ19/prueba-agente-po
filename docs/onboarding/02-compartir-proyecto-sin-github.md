...
> Guía para pasar el proyecto **prueba-agente-po** a otra persona que no tiene acceso a GitHub, para que lo abra en Cursor.
...
```
prueba-agente-po/
├── .cursor/              # Reglas y skills de Cursor (obligatorio)
├── .cursorrules          # Reglas raíz del proyecto
...
2. **Abrir en Cursor**: `File → Open Folder` y seleccionar la carpeta `prueba-agente-po`.
3. **Instalar dependencias**:
   ```bash
   npm install
   npx playwright install
   ```
4. **Configurar plataformas**: Al abrir el proyecto, Cursor (o el agente) seguirá el flujo de primera interacción y creará `Workspace/config/platforms.json` a partir de `docs/templates/platforms.example.json`. La persona debe indicar sus URLs, Jira, Datadog, etc.
